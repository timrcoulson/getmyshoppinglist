...
class PlanService @Inject()(plans: Plans, recipeService: RecipeService, productService: ProductService) {
  def removeRecipe(id: String, recipeId: String): Future[Plan] = {
    for {
      plan <- plans.find(id)
  ...
  def addRecipe(id: String, recipeId: String): Future[Plan] = {
    for {
      plan <- plans.find(id)
  ...
  def swapRecipe(id: String, recipeId: String): Future[Plan] = {
    // Get a new recipe
    val plan = for {
      plan <- plans.find(id)
